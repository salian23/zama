  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="absolute top-3 left-3 eyebrow text-[9px] text-cream/80 bg-ink-950/40 px-2.5 py-0.5 rounded-full backdrop-blur-sm">
          {product.category}
        </span>
        {product.model3d && (
          <button
            onClick={() => setViewer(true)}
            className="absolute top-3 right-3 flex items-center gap-1 rounded-full border border-gold-400/50 bg-ink-950/50 px-2 py-1 text-[9px] uppercase tracking-widest2 text-gold-300 backdrop-blur-sm transition-colors hover:bg-gold-400 hover:text-ink-950"
          >
            <span className="text-xs leading-none">⟳</span> 3D
          </button>
        )}
        <div
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-display text-lg text-cream leading-tight">{product.name}</h3>
          <span className="font-body text-sm text-gold-300 font-semibold whitespace-nowrap">
            ${product.price}
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-1 pt-1">
          {product.notes.map((note) => (
            <span
              key={note}
              className="text-[9px] uppercase tracking-wide text-tea-300 border border-tea-700/60 rounded-full px-2 py-0.5"
            >
              {note}
            </span>
          ))}
        ...
        <button
          onClick={handleAdd}
          className="mt-1 w-full rounded-full border border-gold-400/50 py-2 text-[10px] uppercase tracking-widest2 text-gold-300 transition-all duration-300 hover:bg-gold-400 hover:text-ink-950"
        >
